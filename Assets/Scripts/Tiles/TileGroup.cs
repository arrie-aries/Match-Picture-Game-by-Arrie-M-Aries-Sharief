...
namespace MatchPicture.Tile.Piece
{
    public class TileGroup : MonoBehaviour
    {
        ...
        List<TileObject> update;
        List<FlippedPieces> flipped;
        List<TileObject> dead;
        ...
        void Update()
        {
            List<TileObject> finishedUpdating = new List<TileObject>();
            for (int i = 0; i < update.Count; i++)
            {
                TileObject piece = update[i];
                if (!piece.UpdatePiece()) finishedUpdating.Add(piece);
            }
            for (int i = 0; i < finishedUpdating.Count; i++)
            {
                TileObject piece = finishedUpdating[i];
                FlippedPieces flip = getFlipped(piece);
                TileObject flippedPiece = null;
                ...
                else //If we made a match
                {
                    foreach (Point pnt in connected) //Remove the node pieces connected
                    {
                        ...
                        TileObject nodePiece = node.getPiece();
                        if (nodePiece != null)
                        {
                ...
                for (int y = (height - 1); y >= 0; y--) //Start at the bottom and grab the next
                {
                    ...
                    for (int ny = (y - 1); ny >= -1; ny--)
                    {
                        ...
                        if (nextVal != -1)
                        {
                            ...
                            TileObject piece = gotten.getPiece();
                        ...
                        else//Use dead ones or create new pieces to fill holes (hit a -1) only if we choose to
                        {
                            ...
                            else
                            {
                                ...
                                TileObject n = obj.GetComponent<TileObject>();
                                piece = n;
                            }
        ...
        FlippedPieces getFlipped(TileObject p)
        {
        ...
        void StartGame()
        {
            ...
            update = new List<TileObject>();
            flipped = new List<FlippedPieces>();
            dead = new List<TileObject>();
        ...
        void InstantiateBoard()
        {
            for (int x = 0; x < width; x++)
            {
                for (int y = 0; y < height; y++)
                {
                    ...
                    TileObject piece = p.GetComponent<TileObject>();
        ...
        public void ResetPiece(TileObject piece)
        {
            piece.ResetPosition();
            update.Add(piece);
        }
        ...
        public void FlipPieces(Point one, Point two, bool main)
        {
            ...
            TileObject pieceOne = nodeOne.getPiece();
            if (getValueAtPoint(two) > 0)
            {
                Node nodeTwo = getNodeAtPoint(two);
                TileObject pieceTwo = nodeTwo.getPiece();
    ...
    public class Node
    {
        ...
        public void SetPiece(TileObject p)
        {
            piece = p;
    ...
    {
        public TileObject one;
        public TileObject two;

        public FlippedPieces(TileObject o, TileObject t)
        {
            one = o; two = t;
        }

        public TileObject getOtherPiece(TileObject p)
        {
            if (p == one)
                return two;
            else if (p == two)
